...
def select_policy(Root_insurances):
    ...
    if input(
        ...
        # SECURE: generate a random cryptographically secure policy id with retry logic
        while True:
            Policy_id = secrets.randbelow(9000000) + 1000000 # changed
            if Methods.check_policy_id(Policy_id):
                break
